...
class Database {
    ...
    async userSetDecay(username, date) {
        let res = await this.knex.select("*").from("users").where({ name: username }).insert({ date: date });
        if (res.length)
            return res[0];
        else
            return undefined;
    }
    async updateRole(peer, role) {
        let res = await this.knex.select("*").from("users").where({ name: peer }).update({ role: role });
        if (res.length)
            return res[0];
        else
            return undefined;
    ...
    async guildCreate(guildName, guildLeader, guildLevel, guildMembers) {
    ...
    async guildUpdate(guildName, guildLeader, guildLevel, guildMembers) {
        let res = await this.knex("guilds").where({ name: guildName }).update({ leader: guildLeader, level: guildLevel, members: guildMembers });
    }
    async guildDelete(guildName) {
    }
    async getGuild(guildMembers, guildLeader) {
