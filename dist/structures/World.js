...
class World {
    handlePunch(arg0, peer, base, world) {
    ...
    leave(peer, sendMenu = true) {
        ...
        if (sendMenu)
            peer.send(growtopia_js_1.Variant.from({ delay: 500 }, "OnRequestWorldSelectMenu", "add_filter|\nadd_heading|Top Worlds|\nadd_floater|wotd_world|START|0|0.5|2147418367|"), growtopia_js_1.Variant.from({ delay: 500 }, "OnConsoleMessage", `Where do you want to go?`));
    ...
    async enter(peer, { x, y }) {
        ...
        if (this.worldName === "GROWGANOTH") {
            peer.everyPeer((p) => {
                peer.send(growtopia_js_1.Variant.from("OnSetCurrentWeather", 9));
                p.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, "`2Growganoth is active!`0, Try to drop items and get rare items."));
            });
    ...
    dropGems(peer, x, y, id, amount, { tree, noSimilar } = {}) {
        const tank = growtopia_js_1.TankPacket.from({
            type: TankTypes_1.TankTypes.PEER_DROP,
            netID: -1,
            targetNetID: tree ? -1 : peer.data.netID,
