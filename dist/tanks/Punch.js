...
const game_events_1 = require("./game_events");
const data = new quick_db_1.QuickDB;
/** Handle Punch */
async function handlePunch(tank, peer, base, world) {
    ...
    if (block.fg === 226 && itemMeta.type === Tiles_1.ActionTypes.BOOMBOX) {
        isSignalJammer = true;
        peer.send(growtopia_js_1.Variant.from("OnPlayPositioned", "audio/signal_jammer.wav"));
        peer.send(growtopia_js_1.Variant.from("OnConsoleMessage", "You `4Jammed `0 the world."));
    }
    else if (isSignalJammer = true && itemMeta.type === Tiles_1.ActionTypes.BOOMBOX) {
        peer.send(growtopia_js_1.Variant.from("OnConsoleMessage", "You `2unjammed `0 the world."));
    }
    if (!haveXeno) {
    ...
    if (block.damage === itemMeta.breakHits) {
        ...
        const getRandomPositionInsideRange = (block, rangeX, rangeY) => {
            const minX = block.x * 32;
            const minY = block.y * 32;
    ...
    if (itemMeta.id === 758) {
        ...
        if (randomNum === 0 && hasSpun === false) {
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnTalkBubble", peer.data.netID, `[${peer.data.tankIDName} spun the wheel and got \`2${randomNum}\`0]!`));
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnConsoleMessage", `[${peer.data.tankIDName} spun the wheel and got \`2${randomNum}\`0!]`));
            hasSpun = true;
        }
        if (randomNum < 30) {
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnTalkBubble", peer.data.netID, `[${peer.data.tankIDName} spun the wheel and got \`4${randomNum}\`0]!`));
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnConsoleMessage", `[${peer.data.tankIDName} spun the wheel and got \`4${randomNum}\`0!]`));
            hasSpun = true;
        }
        else {
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnTalkBubble", peer.data.netID, `[${peer.data.tankIDName} spun the wheel and got \`b${randomNum}\`0]!`));
            peer.send(growtopia_js_1.Variant.from({ delay: 2500 }, "OnConsoleMessage", `[${peer.data.tankIDName} spun the wheel and got \`b${randomNum}\`0!]`));
            hasSpun = true;
        }
    ...
    if (world.data.owner) {
        if (world.data.owner.id !== peer.data.id_user) {
            if (peer.data.role !== Constants_1.Role.DEVELOPER) {
                if (itemMeta.id === 242)
                    peer.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, `\`#[\`0\`9World Locked by ${world.data.owner?.displayName}\`#]`));
                return peer.everyPeer((p) => {
                    if (p.data.world === peer.data.world && p.data.world !== "EXIT")
                        p.send(growtopia_js_1.Variant.from({ netID: peer.data.netID }, "OnPlayPositioned", "audio/punch_locked.wav"));
                });
            }
        }
    }
    ...
    if (storedObject && typeof storedObject === 'object' && 'user_id' in storedObject) {
        const { user_id, time } = storedObject;
            ...
            if (world.data.owner) {
                if (typeof world.data.owner.id === user_id) {
                    ...
                    if (itemMeta.id === 242 && timeAsDate.getTime() === oneDayAgo.getTime()) {
                        peer.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, `\`4World Lock has been removed by ${peer.name} due to inactivity\`4\``));
                        peer.everyPeer((p) => {
                            if (p.data.world === peer.data.world && p.data.world !== "EXIT") {
                                p.send(growtopia_js_1.Variant.from({ netID: peer.data.netID }, "OnPlayPositioned", "audio/punch_locked.wav"));
                            }
    ...
    if (itemMeta.id === 8 || itemMeta.id === 6 || itemMeta.id === 3760 || itemMeta.id === 7372) {
        if (peer.data.role !== Constants_1.Role.DEVELOPER && peer.data.role !== Constants_1.Role.ADMIN && peer.data.role !== Constants_1.Role.MOD) {
            peer.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, "It's too strong to break."));
            peer.everyPeer((p) => {
                if (p.data.world === peer.data.world && p.data.world !== "EXIT")
                    p.send(growtopia_js_1.Variant.from({ netID: peer.data.netID }, "OnPlayPositioned", "audio/punch_locked.wav"));
            });
    ...
    if (block.damage >= itemMeta.breakHits) {
        ...
        if (block.fg)
            ...
            block.bg = 0;
        tankData.type = TankTypes_1.TankTypes.TILE_PUNCH;
        tankData.info = 18;
        ...
        tankData.type = TankTypes_1.TankTypes.TILE_DAMAGE;
        ...
        switch (itemMeta.type) {
            case Tiles_1.ActionTypes.SEED: {
                ...
                if (block.tree && Date.now() >= block.tree.fullyGrownAt && game_events_1.gameEvents.Harvestfest) {
                    world.drop(peer, block.x * 32 + Math.floor(Math.random() * 16), block.y * 32 + Math.floor(Math.random() * 16), randomNum, 1);
                }
...
exports.handlePunch = handlePunch;
