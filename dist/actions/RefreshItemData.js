...
class default_1 extends Action_1.Action {
    ...
    handle(base, peer, db, action) {
        peer.send(growtopia_js_1.Variant.from("OnConsoleMessage", "One moment. Updating item data..."), 
        //TankPacket.from({ type: TankTypes.PEER_ITEMS_DAT, data: () => base.items.hash }),
        growtopia_js_1.TankPacket.from({ type: TankTypes_1.TankTypes.PEER_ITEMS_DAT, data: () => base.items.content }));
    }
}
