            ...
            case DataTypes_1.DataTypes.ACTION: {
                ...
                if (parsed?.requestedName && parsed?.tankIDName && parsed?.tankIDPass) {
                    ...
                    base.database.getUser(username).then(async (user) => {
                        ...
                        peer.send(growtopia_js_1.Variant.from("OnSuperMainStartAcceptLogonHrdxs47254722215a", base.items.hash, "ubistatic-a.akamaihd.net", //ubistatic-a.akamaihd.net
                    ...
                    case TankTypes_1.TankTypes.PEER_CLOTH: {
                        ...
                        const isAnces = () => {
                            if (item?.type === Tiles_1.ActionTypes.ANCES || item?.type === Tiles_1.ActionTypes.LOCK) {
                                if (Tiles_1.ActionTypes.LOCK && item.id === 242 || item.id === 1796) {
                                    ...
                                    if (checkPeerInventory?.amount >= 100) {
                                        ...
                                        peer.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, "You compressed 100 World Locks into a `1Diamond Lock`0!"));
                                    }
                                    if (checkPeerInventory1?.amount >= 1) {
                                        ...
                                        //tileUpdate(base, peer, ActionTypes.LOCK, { x: 0, y: 0 }, world!);
                                        // peer.send(Variant.from("OnTalkBubble", peer.data.netID, "You compressed 100 World Locks into a `1Diamond Lock`0!"));
                                        peer.send(growtopia_js_1.Variant.from("OnTalkBubble", peer.data.netID, "You suppressed 1 Diamond Lock into `9100 World Locks`0!"));
                                    }
                                }
